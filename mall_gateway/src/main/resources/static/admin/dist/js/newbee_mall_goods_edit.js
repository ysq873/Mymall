...
$('#saveButton').click(function () {
    ...
    var goodsDetailContent = editorD.txt.html();  //这段代码能将editor里文本框内容转换为html标签字符串传到后端，不论图片还是文字
...
$('#levelOne').on('change', function () {
    $.ajax({
        url: '/admin/categories/listForSelect?categoryId=' + $(this).val(),  //获取当前选择第一分类的id，因为id在初始化页面时已经通过分类列表传入
        type: 'GET',
        success: function (result) {
            if (result.resultCode == 200) {
                var levelTwoSelect = '';
