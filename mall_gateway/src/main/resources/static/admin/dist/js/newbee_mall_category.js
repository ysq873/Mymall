$(function () {
    ...
    $(window).resize(function () {  //这个函数是当浏览器窗口发生变化时触发里面的函数
        $("#jqGrid").setGridWidth($(".card-body").width()); //这个函数是jqgrid的表格宽度会随着浏览器大小变化
    });
});
...
function categoryManage() {
    ...
    var parentId = $("#parentId").val();
    var id = getSelectedRow();  //获取选择的id，就是label上的id
...
$('#saveButton').click(function () {
    ...
    var categoryLevel = $("#categoryLevel").val();
    var parentId = $("#parentId").val(); //在每次get请求时都更新了父id，即在每个分类级别页面都保存有上下这四个值，html用了隐藏文本框来储存数据
...
function categoryEdit() {
    ...
    var rowData = $("#jqGrid").jqGrid("getRowData", id);  //获取改行的数据
    $('.modal-title').html('分类编辑');
    $('#categoryModal').modal('show');
    $("#categoryId").val(id);
    $("#categoryName").val(rowData.categoryName);  //在模态框中显现，因为管理员需要知道这行分类的详细信息。
